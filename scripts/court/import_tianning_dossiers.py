...
"""
天宁法院卷宗导入脚本
import_tianning_dossiers.py

功能：
1. 扫描天宁法院目录下的案件ZIP/7z文件和解压后的文件夹
2. 自动解压ZIP/RAR/7z文件，过滤日志、Excel等非卷宗文件
3. 根据文件名自动识别分类
4. 导入到 court_cases 和 court_dossiers 表

天宁目录结构特点：
- 按日期组织: 2025-09-02/, 2025-09-04/
- ZIP压缩包: 2025苏0402民初4901号2025-08-28.zip
- 已解压文件夹: (2025)苏0402民初4901号/
- 混杂文件: .log, .out, .xlsx, .docx

使用方式：
python3 scripts/court/import_tianning_dossiers.py
python3 scripts/court/import_tianning_dossiers.py --dry-run  # 预览模式
python3 scripts/court/import_tianning_dossiers.py --source /path/to/天宁目录
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('/opt/data_asset/logs/import_tianning.log', encoding='utf-8'),
...
class TianningDossierImporter:
    ...
    def scan_and_import(self, source_dir: str):
        """扫描并导入源目录"""
        logger.info(f"🚀 开始天宁卷宗导入{'（预览模式）' if self.dry_run else ''}")
    ...
    def generate_preview_report(self):
        """生成预览报告（Markdown格式）"""
        report_path = '/opt/data_asset/logs/tianning_preview_report.md'
...
def main():
    parser = argparse.ArgumentParser(description='天宁法院卷宗导入脚本')
    parser.add_argument('--dry-run', action='store_true', help='预览模式，不实际执行')
    parser.add_argument('--no-move', action='store_true', help='仅入库，不移动文件（复制代替移动）')
    parser.add_argument('--source', type=str, required=True, help='天宁卷宗源目录路径')
    parser.add_argument('--config', type=str, default='/opt/data_asset/config.yaml', help='配置文件路径')
    
    args = parser.parse_args()
    
    importer = TianningDossierImporter(config_path=args.config)
