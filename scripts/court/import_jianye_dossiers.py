...
"""
建邺法院卷宗导入脚本
import_jianye_dossiers.py

功能：
1. 扫描建邺法院目录下的案件ZIP文件和解压后的文件夹
2. 自动解压ZIP/7z文件，过滤日志、Excel等非卷宗文件
3. 根据文件名自动识别分类
4. 导入到 court_cases 和 court_dossiers 表

建邺目录结构特点：
- 按日期组织: 2025-01-06/, 2025-01-07/
- ZIP压缩包: (2024)苏0105民初14205号2025-01-06.zip
- 已解压文件夹: (2024)苏0105民初14205号2025-01-06/
- 混杂文件: .log, .out, .xlsx, .7z

使用方式：
python3 scripts/court/import_jianye_dossiers.py
python3 scripts/court/import_jianye_dossiers.py --dry-run  # 预览模式
python3 scripts/court/import_jianye_dossiers.py --source /path/to/建邺目录
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('/opt/data_asset/logs/import_jianye.log', encoding='utf-8'),
...
class JianyeDossierImporter:
    ...
    def scan_and_import(self, source_dir: str):
        """扫描并导入源目录"""
        logger.info(f"🚀 开始建邺卷宗导入{'（预览模式）' if self.dry_run else ''}")
    ...
    def generate_preview_report(self):
        """生成预览报告（Markdown格式）"""
        report_path = '/opt/data_asset/logs/jianye_preview_report.md'
...
def main():
    parser = argparse.ArgumentParser(description='建邺法院卷宗导入脚本')
    parser.add_argument('--dry-run', action='store_true', help='预览模式，不实际执行')
    parser.add_argument('--no-move', action='store_true', help='仅入库，不移动文件（复制代替移动）')
    parser.add_argument('--source', type=str, required=True, help='建邺卷宗源目录路径')
    parser.add_argument('--config', type=str, default='/opt/data_asset/config.yaml', help='配置文件路径')
    
    args = parser.parse_args()
    
    importer = JianyeDossierImporter(config_path=args.config)
