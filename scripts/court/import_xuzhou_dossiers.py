...
"""
徐州法院卷宗导入脚本
import_xuzhou_dossiers.py

功能：
1. 扫描徐州法院目录下的案件ZIP/7z文件和解压后的文件夹
...
徐州目录结构特点：
- 法院代码: 0311(泉山)
- ZIP压缩包: 2025苏0311民初23号2025-06-16.zip
- 已解压文件夹: (2025)苏 0311 民初 819 号/
- 混杂文件: .log, .docx, .lnk

使用方式：
python3 scripts/court/import_xuzhou_dossiers.py
python3 scripts/court/import_xuzhou_dossiers.py --dry-run  # 预览模式
python3 scripts/court/import_xuzhou_dossiers.py --source /path/to/徐州目录
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('/opt/data_asset/logs/import_xuzhou.log', encoding='utf-8'),
...
class XuzhouDossierImporter:
    ...
    def scan_and_import(self, source_dir: str):
        """扫描并导入源目录"""
        logger.info(f"🚀 开始徐州卷宗导入{'（预览模式）' if self.dry_run else ''}")
    ...
    def generate_preview_report(self):
        """生成预览报告（Markdown格式）"""
        report_path = '/opt/data_asset/logs/xuzhou_preview_report.md'
...
def main():
    parser = argparse.ArgumentParser(description='徐州法院卷宗导入脚本')
    parser.add_argument('--dry-run', action='store_true', help='预览模式，不实际执行')
    parser.add_argument('--no-move', action='store_true', help='仅入库，不移动文件（复制代替移动）')
    parser.add_argument('--source', type=str, required=True, help='徐州卷宗源目录路径')
    parser.add_argument('--config', type=str, default='/opt/data_asset/config.yaml', help='配置文件路径')
    
    args = parser.parse_args()
    
    importer = XuzhouDossierImporter(config_path=args.config)
